...
class growth():
    ...
    from .setpoint import return_setpoint
    ...
    def __init__(self,growth_params,initial_numbers_of_hs,data_buffer_size):
        ...
        start_index = int(growth["start_index"][0])

        self.ventricle_wall_volume = float(growth["ventricle_wall_volume"][0])

        # wall_thickness
        self.ventricle_slack_volume = float(growth["ventricle_slack_volume"][0])
        ...
        if growth["driven_signal"][0] == "strain":
            self.G_n_hs = float(growth["strain_signal"]["eccentric_growth"]["Gs_number_of_hs"][0])
